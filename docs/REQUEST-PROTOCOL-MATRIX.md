...
| `/api/v1/usage` | POST | `Authorization: Bearer` | body 使用对应账号的 `fingerprintId` | `fingerprintId` 只在此 body 出现，不进入 chat、session 或 agent-runs |
...
| `id`、`name`、`email`、`fingerprintHash` | 不属于 VPS 规范配置，当前适配器不使用 | 不加入任何上游请求 |
