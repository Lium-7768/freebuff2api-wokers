...
| `/api/v1/usage` | POST | `Authorization: Bearer` | body 的 `fingerprintId` 优先级为显式 `FREEBUFF_FINGERPRINT_ID`、账号 credentials 的 `fingerprintId`、`cli-usage` | `fingerprintId` 只在此 body 出现，不进入 chat、session 或 agent-runs |
