...
当前 VPS 若仍安装旧版 `deploy/freebuff2api-deploy`，首次切换前需用本文件同名脚本替换 root-owned forced-command 入口。旧脚本会检查 `upstream_tokens`，新版只接受 `upstream_credentials_json`。脚本更新后，后续只需更新 GitHub `Production` secrets 并重新运行工作流；不需要手工修改容器环境变量。
