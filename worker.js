...
function findManusModel(modelId) {
  const raw = String(modelId || "").trim();
  return MANUS_MODELS.find((model) => model.id === raw) || null;
}
...
function manusSseResponse(model, content) {
...
async function handleManusChat(request, env, params, manusModel) {
...
async function handleChat(request, env) {
  ...
  const manusModel = findManusModel(requestedModel);
  if (manusModel) return handleManusChat(request, env, params, manusModel);
...
async function handleModels(env) {
  ...
  const manusModels = String(env.MANUS_API_KEY || "").trim() ? MANUS_MODELS : [];
